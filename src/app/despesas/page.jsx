...
export default function ExpensesPage() {
  ...
  const expenses = useMemo(() => {
    return mockTransactions.filter((t) => t.type === "expense");
  }, [])

  // Aplicar filtros de data e busca
  const filteredExpenses = useMemo(() => {
    let filtered = filterTransactionsByDate(expenses, activeFilter.startDate, activeFilter.endDate)
  ...
  const expensesByCategory = useMemo(() => {
    return groupTransactionsByCategory(filteredExpenses);
  }, [filteredExpenses])

  // Dados para gráfico
  const chartData = useMemo(() => {
    return expensesByCategory.map((group) => ({
      name: group.category,
      value: group.total,
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card className="rounded-2xl border-slate-200 shadow-sm">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-slate-900">{filteredExpenses.length}</div>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-slate-900">
              {new Intl.NumberFormat("pt-BR", {
                style: "currency",
                currency: "BRL",
              }).format(filteredExpenses.length > 0 ? totalExpenses / filteredExpenses.length : 0)}
            </div>
              ...
              {expensesByCategory.slice(0, 6).map((category, index) => (
                <div key={category.category} className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-sm text-slate-500">{((category.total / totalExpenses) * 100).toFixed(1)}%</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <Card className="rounded-2xl border-slate-200">
        <CardHeader>
          <div
            className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              <CardTitle className="text-slate-900">Lista de Despesas</CardTitle>
              <CardDescription>Todas as despesas do período selecionado</CardDescription>
            </div>
